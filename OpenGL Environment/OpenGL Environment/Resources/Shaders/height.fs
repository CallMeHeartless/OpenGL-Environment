...
void main(void){
	...
	vec4 colour1 = vec4(1.0f, 1.0f, 0.0f, 1.0f);
	vec4 colour2 = vec4(0.0f, 1.0f, 1.0f, 1.0f);
	float height = fragPos.y/maxHeight;
	//color = mix(colour1, colour2, height);
	...
	color = vec4(ambient + diffuse + specular, 1.0f) *  mix(colour1, colour2, height);//texture(tex, fragTexCoord);
